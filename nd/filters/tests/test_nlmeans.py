...
def test_nlmeans():
    ds_nlm = NLMeansFilter(
        dims=('lat', 'lon'), r=0, f=1, sigma=2, h=2).apply(ds)
...
def test_nlmeans_zero_radius():
    ...
    ds_nlm = NLMeansFilter(dims=('lat', 'lon'), r=0, f=1, sigma=1, h=1).apply(ds)
...
def test_reduce_std():
    ...
    ds_nlm = NLMeansFilter(
        dims=('lat', 'lon', 'time'), r=(1, 1, 0), sigma=2, h=2).apply(ds)
...
def test_ignore_time_dimension():
    ...
    t0_nlm = NLMeansFilter(
        dims=('lat', 'lon'), r=1, sigma=2, h=2).apply(t0)

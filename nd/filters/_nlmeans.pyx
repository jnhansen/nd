...
import numpy as np
cimport numpy as np
from libc.math cimport abs, log, isnan, exp
...
cdef double _dsquare(floating [:, :, :] Bp, floating [:, :, :] Bq):
    cdef:
        SIZE_TYPE nrows = Bp.shape[0]
        SIZE_TYPE ncols = Bp.shape[1]
        SIZE_TYPE nvars = Bp.shape[2]
        SIZE_TYPE i, j, v
        double dsquare = 0
    ...
    for i in range(nrows):
        for j in range(ncols):
            for v in range(nvars):
                dsquare += (Bp[i, j, v] - Bq[i, j, v]) ** 2

    dsquare /= (nrows * ncols * nvars)
